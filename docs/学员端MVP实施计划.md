...
| T5-04 验证 | 部分完成 | 前端生产构建、增量类型检查和接口契约检查通过；真实 PDF 鉴权预览文件流已通过，引用页码联动仍待 PDF 解析入库及真实模型验证 |
...
| T7-03 PDF/真实模型/性能 | 部分完成 | 真实 PDF 预览及下载策略通过；20 并发全部成功，我的课程/目录/资料列表 P95 分别为 59.8/57.8/165.6ms，PDF 首字节 P95 为 50.7ms；模型网关配置缺失，真实回答及 PDF 引用页码联动仍待完成 |
