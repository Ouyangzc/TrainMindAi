...
总体状态：学员端 MVP 已完成验收。当前阶段引用验收以“可从回答进入对应课程资料”为完成标准，精确 PDF 页码定位作为后续增强项。
...
| T3 验证 | 已完成 | 数据库迁移、前后端构建、Spring 启动和 OpenAPI 路由注册通过；真实 PDF 返回 `application/pdf`、inline、no-store 及正确 `%PDF-` 文件头，下载关闭时返回 403 |
...
| T4 验证 | 已完成 | 三张业务表迁移、AI 定向测试与 Ruff、Java 构建、Spring/MyBatis 启动及 OpenAPI 路由注册通过；真实模型已能基于课程资料回答并返回对应资料引用 |
...
| T5-03 验证 | 已完成 | 前端生产构建、增量类型检查和前后端契约核对通过；真实模型回答、历史会话和引用资料跳转完成页面验收 |
| T5-04 课程目录与资料库 | 已完成 | 接入当前发布快照目录、资料搜索、模块与类型筛选及分页；支持 AI 引用资料高亮、PDF Blob 鉴权预览与页码定位，并按课程开关控制下载入口 |
| T5-04 验证 | 已完成 | 前端生产构建、增量类型检查和接口契约检查通过；真实 PDF 鉴权预览文件流及从 AI 引用进入对应课程资料均通过 |
...
| T7-03 PDF/真实模型/性能 | 已完成 | 真实模型课程问答与对应资料引用通过；真实 PDF 预览及下载策略通过；20 并发全部成功，我的课程/目录/资料列表 P95 分别为 59.8/57.8/165.6ms，PDF 首字节 P95 为 50.7ms |
