...
Expected: `10`（4 个 coalesce + 6 个 "query"）。
...
> 若执行环境无 PG，此步标记为「待执行」，由具备 PG 环境者运行；脚本仍需通过下一步的静态检查。

- [ ] **Step 5: 静态检查无 MySQL 残留语法**
...
- [ ] **Step 2: 保留末尾初始化 insert（若有）**

`quartz.sql` 末尾若有 `commit;` 或初始数据，按 PG 语法保留（PG 支持 `commit;`，但脚本中多为 DDL，通常无需显式提交）。

- [ ] **Step 3: 在 PostgreSQL 中执行脚本验证**

Run: `psql -h localhost -U postgres -d ry-vue -v ON_ERROR_STOP=1 -f sql/quartz_postgresql.sql`
Expected: 无 ERROR；所有 QRTZ_* 表创建成功。
> 无 PG 环境时同 Task 7 Step 4，标记「待执行」。

- [ ] **Step 4: 静态检查无 MySQL 残留语法**

Run: `grep -niE 'auto_increment|engine=|`|blob|tinyint|datetime' sql/quartz_postgresql.sql`
Expected: 无输出（退出码 1；`blob` 应已全部改为 `bytea`）。
